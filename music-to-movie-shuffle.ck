//------------------------------------------------------------------------------
// name: music-to-movie-shuffle.ck
// desc: feature-based synthesizer that takes song as
//       song as input and matches user-chosen dance
//       clips to 1-nearest audio features.
//
// version: need chuck version 1.4.2.1 or higher
// sorting: part of ChAI (ChucK for AI)
//
// USAGE: run with INPUT model file
//        > chuck music-to-movie-shuffle.ck:path/to/song.wav (optional)
//
...
"data/video_features.txt" => string FEATURES_FILE;
"" => string SONG_CHOICE;
// if have arguments, override filename
if( me.args() > 0 )
{
    me.arg(0) => SONG_CHOICE;
}
//------------------------------------------------------------------------------
...
fft =^ MFCC mfcc =^ combo;
// add spectral feature: RollOff
fft =^ RollOff roll =^ combo;

// if we want to hear our audio input
input => Delay delay => Gain g => dac.left => dac.right;
// add artificial delay for time alignment to mosaic output
0::ms => delay.max => delay.delay;
// scale the volume
1 => g.gain;

0 => int default;
// ATTENTION! Change the following index to choose one of the pre-assigned songs
1 => int SONG_INDEX;
[174, 148, 135, 146] @=> int BPMs[];
["data/leavemealone.wav", "data/diana.wav", "data/party.wav", "data/drake.wav"] @=> string songs[];
if (SONG_CHOICE != "")
{
    SONG_CHOICE => input.read;
}
else
{
    // change the following to choose from list of songs above :^)
    // songs[0] - leavemealone by Fred Again..
    songs[SONG_INDEX] => input.read;
    // indicate default option
    1 => default;
}
...
{
    // BPM of leavemealone
    // BPMs[CHOICE] * 4 => float BPM;
    BPMs[SONG_INDEX] * 4 => float BPM;
    
    44100 => int srate;
    60.0 / BPM => float beatDuration;
    srate * beatDuration => float samplesPerBeat;
    
    Math.pow(2, Math.ceil(Math.log2(samplesPerBeat))) $ int => int fftSize;
    // Adjust FFT settings
    fftSize => fft.size;
    Windowing.hann(fft.size()) => fft.window;
    (fft.size() / 2)::samp => HOP;
    4 => NUM_FRAMES;
    beatDuration::second => EXTRACT_TIME;
    <<< EXTRACT_TIME >>>;    
}
else
{
    ...
    fft.size()::samp * NUM_FRAMES => EXTRACT_TIME;
    <<< EXTRACT_TIME >>>;    
}
...
//------------------------------------------------------------------------------
KNN2 knn;
// 1-nearest neighbor (lower k => less randomness)
1 => int K;
...
["singing in the rain", "pulp fiction", "napoleon dynamite", "saturday night fever", "la la land",
"charlie brown", "intouchables", "sound of music", "skeleton dance"] @=> string titles[];
0 => int timeIndex;
...
fun void synthesize( int uid )
{
    ...
    if( !MODE_LET_PLAY) sendWindow( videoFileIndex, win.windowTime );
        ...
        while( hid.recv( msg ) )
        {
            ...
            if( msg.isButtonDown() ) // button down
            {
                ...
                else if( msg.which == 80)  // <-
                {
                    if (videoFileIndex <= 0)
                    {
                        numVideos => videoFileIndex;
                    }
                    (videoFileIndex - 1) % numVideos => videoFileIndex;
                }
                else if( msg.ascii == 82 )  // r
                {
                    sendRainbowEffect();
                }
            }
...
// Sends rainbow tint effect to Processing
fun void sendRainbowEffect()
{
    // start the message
    xmit.start( "/mosaic/rainbow" );
    // send it
    xmit.send();
}
